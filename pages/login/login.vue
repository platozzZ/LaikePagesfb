    ...
    <div v-if="showCMMC">
        <div style='display: flex;flex-direction: column;min-height: 100vh;'>
            <div class="content">
                <!--登录页面-->
                <div class="data_h "
                     :style="{height: halfWidth}">
                    <div class="data_h_h"
                         :style="{height: halfWidth}"></div>
                </div>

                <div :style="{position:'relative',top:baiduHeadTop + 'px'}">

                    <div class="head">
                        <img class='head_close'
                             :src="guanbi"
                             @tap="_banck" />
                        <div @tap="_register_q()">
                            注册
                    ...
                    <div class='login'
                        ...
                        <div class='login_inpu'>
                            <input type="text"
                                   :placeholder="passLoginCodePH"
                                   v-model='account'
                                   placeholder-style="color:#b8b8b8"
                                   @blur="_noNull(1)" />
                            <img :src="del"
                                 v-show="account.length && account_f"
                                 @tap="_empty(1)" />
                        </div>
                        ...
                        <div class='login_inpu'>
                            <input type="text"
                                   ...
                                   :placeholder="passLoginPWPH"
                                   v-model='password'
                                   placeholder-style="color:#b8b8b8"
                                   @blur='_noNull(1)' />
                            <img :src="LoginPWStatus?pwHide:pwShow"
                                 style='height: 32upx;width: 32upx;right: 20upx;bottom: 24upx;'
                                 @tap="pwStatus(1)" />
                        </div>

                        <p class='login_pass'
                           v-if="landing"><span @tap='_navigateTo("retrievepassword")'>忘记密码？</span></p>
                        <div class='button1'
                             style='margin-top: 70upx;'
                             v-if='pwLoginBtnStatus'
                             @tap="_landing">登录</div>
                        <div class='button1'
                             v-else
                             style='opacity: 0.4;margin-top: 70upx;'>登录</div>
                        <div style='text-align: center;font-size: 28upx;color: #999999;'>或</div>
                        <div class='button2'
                             @tap="_phone">验证码登录</div>
                    </div>
                    ...
                    <div class='login'
                        ...
                        <div class='login_inpu'
                            ...
                            <input type="number"
                                   @focus='_codeF()'
                                   @blur='_codeB()'
                                   :placeholder="codeLoginPWPH"
                                   v-model="phone_code"
                                   placeholder-style="color:#b8b8b8"
                                   @input='_codeChangeCode'
                                   maxlength="6" />
                            <p class='login_p'
                               style='z-index: 99;position: absolute;right: 24upx;'
                               @tap="_phone_code(1)"
                        ...
                        <div style='text-align: center;font-size: 28upx;color: #999999;'>或</div>
                        <div class='button2'
                             @tap="_landing_passw">密码登录</div>
                    </div>
                </div>
            </div>
            <div class='agreement'
                 @tap="_navigateTo('agreement')">注册/登录即代表已同意<span style="text-decoration:underline">《{{agreement1}}》</span></div>
        </div>
    </div>
</template>
...
export default {
    data() {
        return {
            ...
            showCMMC: !uni.getStorageSync("tripartiteChannel").toLowerCase()

        }
    },
    onLoad(option) {

        if (uni.getStorageSync("tripartiteChannel")) {
            //todo 配置
